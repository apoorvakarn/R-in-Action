...
abline(reg = lm(TSdata~time(TSdata)))#time(tsdata) means point of time data
cycle(TSdata)
...
plot(diff(TSdata,differences = 1),ylab="Diff(Sales)",xlab="Year",main="Diff(Sales) between 2003-2014",col="green")
...
plot(diff(log10(TSdata),differences = 1),ylab="Diff(Sales)",xlab="Year",main="Diff(Log(Sales)) between 2003-2014",col="brown")
...
par(mfrow=c(1,2))
acf(diff(log10(TSdata)),main="ACF plot",col ="red")#ACF PLOT -- Moving Average or q
pacf(diff(log10(TSdata)),main="PACF plot",col="brown")#PACF PLOT -- Auto Regressive or p
#Running the ARIMA model
ARIMAFit=arima((log10(TSdata)),c(1,1,0))#c(1,1,0)means order of ARIMA(AR[p],I[d],MA[q])
summary(ARIMAFit)
...
par(mfrow=c(1,1))
plot(TSdata,type="l",xlim=c(2004,2018),ylim=c(1,1600),xlab="Year",ylab="Sales",col="green")
